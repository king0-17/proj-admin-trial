...
const actions = {
    getList({ commit, rootGetters, dispatch }, pl) {
        return new Promise(async function(resolve) {
            const f = await fetch('http://localhost:8080/sample_data/domain.json')
                .then((response) => response.json())

            commit('setData', f)

            // $api.post(`/domain/list${pl.page_no ? '?page=' + pl.page_no : ''}`, pl, {
    ...
    updatePassword({ rootGetters, dispatch }, pl) {
        return new Promise(function(resolve) {
            ...
            $api.post('/domain/update-password', pl, {
                    headers: { 'Authorization': rootGetters['auth/token_bearer'] }
                })
                .then(function(res) {
                    if (res.data.status == 1) {
    ...
    updateStatus({ rootGetters, dispatch }, pl) {
        return new Promise(function(resolve) {
            ...
            $api.post('/domain/update-status', pl, {
                    headers: { 'Authorization': rootGetters['auth/token_bearer'] }
                })
                .then(function(res) {
                    if (res.data.status == 1) {
